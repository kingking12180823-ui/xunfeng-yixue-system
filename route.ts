...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!body.question || body.question.trim().length < 2) {
      return json({ ok: false, error: "請輸入問題。" }, 400);
    }
    ...
    const [openaiFirst, geminiFirst, deepseekFirst] = await Promise.all([
      callOpenAI("openaiFengYi", "ChatGPT 風羿分身", openaiFengYiSystem(), firstRoundPrompt(input)),
      callGemini("geminiFengYi", "Gemini 風羿分身", geminiFengYiSystem(), firstRoundPrompt(input)),
      callDeepSeek("deepseekAttack", "DeepSeek 攻防反證", deepseekAttackSystem(), firstRoundPrompt(input)),
    ]);

    const firstRoundText = stringifyRound("第一輪：四術初判", [openaiFirst, geminiFirst, deepseekFirst]);
    ...
    if ((process.env.ENABLE_DEBATE_ROUND || "true").toLowerCase() === "true") {
      const [openaiDebate, geminiDebate, deepseekDebate] = await Promise.all([
        callOpenAI("openaiFengYi", "ChatGPT 風羿分身｜攻防修正", openaiFengYiSystem(), debatePrompt(input, firstRoundText)),
        callGemini("geminiFengYi", "Gemini 風羿分身｜攻防修正", geminiFengYiSystem(), debatePrompt(input, firstRoundText)),
        callDeepSeek("deepseekAttack", "DeepSeek 攻防反證｜第二輪", deepseekAttackSystem(), debatePrompt(input, firstRoundText)),
      ]);
...
4. 由風羿老師本人進行最後人工覆核。`;
    ...
    return json({ ok: false, error: error?.message || "伺服器發生錯誤" }, 500);
  }
}
